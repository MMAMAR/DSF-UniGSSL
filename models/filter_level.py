...
def proximity_mean_std(z: torch.Tensor, args, filtering_threshold, pca_n_comp):
    ...
    #mask = (torch.abs(z_project - mean) < args["disk"] * std).all(dim=1).cpu()
    
    mask = (torch.abs(z_project - mean) < filtering_threshold * std).all(dim=1)
...
def proximity_level_semi_loss(z1: torch.Tensor, z2: torch.Tensor, z3: torch.Tensor, args) -> torch.Tensor:
    ...
    #p1 = self.proximity(z1, args).t().to(device)
    ...
    with tqdm(total=finetuning_args['num_epochs'], desc='(T)') as pbar:
        for epoch in range(1, finetuning_args['num_epochs'] + 1):
            ...
            if finetuning_type == 'prox':
                pretrained_indices, finetuning_indices = proximity_mean_std(z1, finetuning_args, filtering_threshold, pca_n_comp)
            elif finetuning_type == 'cluster':
                pretrained_indices, finetuning_indices = cluster_mean_std(z1, finetuning_args, filtering_threshold, pca_n_comp)
            elif finetuning_type == "graph":
                pretrained_indices, finetuning_indices = graph_mean_std(z1, finetuning_args, filtering_threshold, pca_n_comp)
            ...
            loss = loss_pretraining + 3*loss_finetuning
            loss.backward()
            optimizer.step()
