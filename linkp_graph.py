...
from torch_geometric.utils import dropout_adj, negative_sampling, to_undirected
import numpy as np
...
parser.add_argument("--device_number", type=int, default=3)
parser.add_argument("--config", type=str, default="config/config-graph-linkp.yml")
parser.add_argument("--nb_runs", type=int, default=2)
args = parser.parse_args()

if not args.pretrained:
    save_path = "pretrained_models/{}/linkp/graph.pth".format(args.dataset)
    pretrained_path = "{}"

else:
    save_path = "finetuned_models/{}/linkp/{}_graph.pth".format(args.dataset, args.pretrained_type)
...
drop_feature_rate_2 = config["drop_feature_rate_2"]#0.2
...
if not args.pretrained:
    print(f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {args.dataset}, SingleTask-graph, Epoch:{best_epoch}, AUC-mean: {np.mean(auc_scores) * 100}, AUC-std: {np.std(auc_scores) * 100}' )
else:
    print(f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {args.dataset}, DualStage-({args.pretrained_type}, graph), Epoch:{best_epoch}, AUC-mean: {np.mean(auc_scores) * 100}, AUC-std: {np.std(auc_scores) * 100}' )